        ...
        if start == 1:
            if "teleport" in trigger[
                ...
                if door_type != "interior":
                    if target_loc in ["#Overworld", "#Underworld", "#Moon"] and loc in ["#LunarPassage1","#LunarPassage2","#MistCave","#Mist"]:
                        if number in ["4","1"]:
                            facing="up"
                        elif number=="7":
                            facing="right"
                            ...
                            facing="down"
                    triggers.append(
                        [loc, number, x, y, target_loc, target_x, target_y, facing, door_type, f"{loc}_{target_loc}_{facing}",
                         map_key[target_loc]])
        trigger = [line]
        start = 1
    else:
...
to_remove = ["#SoldierAirship", "#GiantMouth","#MysidiaSerpentRoad",'#BlackChocoboForest',"#SealedCaveEntrance",'#AdamantGrotto','#CaveEblanEntrance']
...
hardcoded = [["#Underworld", "5", "48", "15", "#Babil1F", "15", "24", "up", "entrance", "#Underworld_#Babil1F_up",
             "#Underworld"],["#Underworld", "6", "49", "15", "#Babil1F", "15", "24", "up", "entrance", "#Underworld_#Babil1F_up",
             "#Underworld"],
             ["#Silvera", "7", "17", "31", "#Overworld", "210", "130", "", "exit","#Silvera_#Overworld_","#Overworld"]
             ]
